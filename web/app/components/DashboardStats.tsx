...
const iconMap: Record<string, JSX.Element> = {
  ...
  testinglabs: <FontAwesomeIcon icon={faVial} />,
  bloodbanks: <FontAwesomeIcon icon={faDroplet} />,
};
...
const DashboardStats = () => {
  ...
  return (
    <div className="bg-violet-300 m-4 p-4 rounded-3xl">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {Object.entries(stats).map(([category, count]) => (
          <div
            key={category}
            className="bg-slate-400 hover:bg-white text-white hover:text-slate-500 p-4 mb-4 rounded-3xl"
          >
            <Link href={`/dashboard?status=${category}`}>
              ...
              <div className="text-slate-800 text-lg font-semibold capitalize">
                {category}
              </div>
            </Link>
          </div>
